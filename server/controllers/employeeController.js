...
const createEmployee = async (req, res) => {
  try {
    ...
    console.log('Looking for department with ID:', department); // Debug log
    const departmentExists = await Department.findById(department);
    console.log('Department found:', departmentExists); // Debug log
    if (!departmentExists) {
      return res.status(400).json({
        success: false,
    ...
    res.status(500).json({
      success: false,
      error: `Server Error - Failed to create employee: ${error.message}`
    });
  }
};
